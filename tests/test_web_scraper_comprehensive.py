...
class TestWebScraperComprehensive:
    ...
    def test_extract_text_content_semantic_elements(self) -> None:
        ...
        with patch('src.rag_sample.web_scraper.BeautifulSoup') as mock_bs:
            mock_soup = Mock()
            mock_article = Mock()
            mock_article.get_text.return_value = "Main Title\nMain content"
            mock_soup.find.return_value = mock_article
            mock_bs.return_value = mock_soup
    ...
    def test_extract_text_content_common_patterns(self) -> None:
        ...
        with patch('src.rag_sample.web_scraper.BeautifulSoup') as mock_bs:
            mock_soup = Mock()
            mock_content = Mock()
            mock_content.get_text.return_value = "Title\nContent"
            mock_soup.find.return_value = mock_content
            mock_bs.return_value = mock_soup
    ...
    def test_extract_text_content_largest_text_block(self) -> None:
        ...
        with patch('src.rag_sample.web_scraper.BeautifulSoup') as mock_bs:
            ...
            mock_soup.find_all.return_value = [
                Mock(get_text=lambda: "Small content"),
                Mock(get_text=lambda: "This is a much longer content block that should be selected"),
                Mock(get_text=lambda: "Another small content")
            ]
            mock_bs.return_value = mock_soup
            
            result = self.scraper._extract_text_content(html)
            assert "This is a much longer content block that should be selected" in result
    ...
    def test_extract_text_content_body_fallback(self) -> None:
        ...
        with patch('src.rag_sample.web_scraper.BeautifulSoup') as mock_bs:
            mock_soup = Mock()
            mock_body = Mock()
            mock_body.get_text.return_value = "Title\nContent"
    ...
    def test_extract_text_content_error_handling(self) -> None:
        ...
        with patch('src.rag_sample.web_scraper.BeautifulSoup', side_effect=Exception("Parsing error")):
            result = self.scraper._extract_text_content("<html>Invalid HTML</html>")
            assert result == ""
